...
class OrganizationProcessingIssuesEndpoint(OrganizationEndpoint):
    def get(self, request, organization):
        ...
        """
        projects = Project.objects.filter(id__in=self.get_project_ids(request, organization))
        data = get_processing_issues(request.user, projects, request.GET.get('detailed') == '1')
